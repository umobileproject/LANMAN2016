...
main(int argc, char* argv[])
{
  ...
  uint32_t num_contents = 10;  //total number of contents
  double connection_rate = 1.0; //rate at which users connect (i.e., send interest)
  double disconnection_rate = 0.2; //rate at which users disconnect (i.e., go offline)
  //double zipf_exponent ; //for content popularity
  double initialization_period_length = 40.0;
  double observation_period_length = 200.0;
  double connect_time = 0.2;
  double disconnect_time = 0.2;
  double zipf_exponent = 0.7;
  int cache_size = num_contents/5;
